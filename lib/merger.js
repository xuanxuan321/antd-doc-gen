...
function generateIndexFile(docPaths, outputDir) {
  ...
  docPaths.forEach((docPath) => {
    ...
    if (componentIndex !== -1 && componentIndex + 1 < pathParts.length) {
      ...
      // 如果没有找到，使用倒数第二个部分作为组件名
      componentName = formatComponentName(pathParts[pathParts.length - 2]);
    }

    if (componentName) {
      componentMap.set(componentName, `./components/${componentName}.md`);
    }
  });
  ...
  sortedComponents.forEach((componentName) => {
    const docPath = componentMap.get(componentName);
    indexContent += `- [${componentName}](${docPath})\n`;
  });
